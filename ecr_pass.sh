...
data="{}"
data_hash=$(echo -n $data | openssl dgst -sha256)
cr="POST\n/\n\ncontent-type:application/x-amz-json-1.1\nhost:api.ecr.${region}.amazonaws.com\nx-amz-date:${time_now}\nx-amz-target:AmazonEC2ContainerRegistry_V20150921.GetAuthorizationToken\n\ncontent-type;host;x-amz-date;x-amz-target\n${data_hash}"
cr_hash=$(echo -ne $cr | openssl dgst -sha256)
string_to_sign="AWS4-HMAC-SHA256\n${time_now}\n${day_now}/${region}/ecr/aws4_request\n${cr_hash}"
k_date=$(echo -n $day_now | openssl dgst -sha256 -hmac "AWS4${secret}")
k_region=$(echo -n $region | openssl dgst -sha256 -mac HMAC -macopt hexkey:${k_date})
k_service=$(echo -n "ecr" | openssl dgst -sha256 -mac HMAC -macopt hexkey:${k_region})
k_signing=$(echo -n "aws4_request" | openssl dgst -sha256 -mac HMAC -macopt hexkey:${k_service})
signature=$(echo -ne $string_to_sign | openssl dgst -sha256 -mac HMAC -macopt hexkey:${k_signing})


responce=$(curl -s --data "{}" \
